...
export const CONTEXT_PROVIDER = ({children}) => {
    ...
    const GET_POOL_ADDRESS = async(liqudity, selectedNetwork) => {
        try {
            setLoad(true);
            // PROVIDER
            const PROVIDER = new ethers.providers.JsonRpcBatchProvider(selectedNetwork.rpcUrl);
            // const PROVIDER = new ethers.providers.JsonRpcBatchProvider("https://rpc.ankr.com/polygon_mumbai");
            const factoryContract = new ethers.Contract(FACTORY_ADDRESS, FACTORY_ABI, PROVIDER);
                
            const poolAddress  = await factoryContract.functions.getPool(liqudity.token_A, liqudity.token_B, Number(liqudity.fee));
            const PoolHistory = {
    ...
    const GET_POOL_DETAILS = async(poolAddress, selectedNetwork) => {
        try {
            ...
            const poolContract = new Contract(
                poolAddress, UniswapV3Pool.abi, PROVIDER
            );
    ...
    return <CONTEXT.Provider value={{DAPP_NAME, GET_POOL_ADDRESS, load, GET_POOL_DETAILS}}>
        {children}
    </CONTEXT.Provider>
}
